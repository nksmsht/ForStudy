...
#------------------------------------------------------------------------------------------------------------------------------
    ...
    if f[5] != ("" and "EMPTY"):
        X = int(f[5])
        Y = int(f[6])
        cv2.circle(lined_img, (X, Y), 40, (0,0,255), thickness=30, lineType=cv2.LINE_4)
        

    imgName = output_dir + "/image" + str('{0:05d}'.format(count)) + ".jpg"
    lined_img = cv2.resize(lined_img, (int(width*0.285), int(height*0.285)))
